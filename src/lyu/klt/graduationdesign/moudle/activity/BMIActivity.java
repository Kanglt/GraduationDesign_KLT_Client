...
public class BMIActivity extends BaseActivity {
	...
	public void startGame() {
		...
		UserBodyDataAPI.getUserBodyData(context, AbSharedUtil.getString(context, Constant.LAST_LOGINID), "BMI", getUserBodyDataStringHttpResponseListener);
	}
	...
	private LineData LineData(int count) {
		...
		mlinechart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
			...
			//	tv_userWeight_with_lastTime.setText(userBodyDataPoList.get(e.getXIndex()).getDate());
				...
				float x=0;
				if(e.getXIndex()>0){
					x=Float.parseFloat(userBodyDataPoList.get(e.getXIndex()).getWeight())-Float.parseFloat(userBodyDataPoList.get(e.getXIndex()-1).getWeight());
				}
			...
			public void onNothingSelected() {
				...
				//	tv_userWeight_with_lastTime.setText(userBodyDataPoList.get(e.getXIndex()).getDate());
					...
					float x=0;
					x=Float.parseFloat(userBodyDataPoList.get(userBodyDataPoList.size()-1).getWeight())-Float.parseFloat(userBodyDataPoList.get(userBodyDataPoList.size()-2).getWeight());
		...
		public void onSuccess(int statusCode, String content) {
			...
			if (!StringUtil.isEmpty(content)) {
				try {
					...
					userBodyDataPoList = gson.fromJson(jsonObject.getString("list"), new TypeToken<List<UserBodyDataPo>>() {
					}.getType());
