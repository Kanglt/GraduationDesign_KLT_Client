...
public class BodyFatRateActivity extends BaseActivity {
	...
	private static final String TAG = BodyFatRateActivity.class.getSimpleName();
	...
	private TextView tv_BFR_date;
	private TextView tv_BFR;
	private EditText edi_BMI_weight, edi_BFR_waist_circumference;
	private Button btn_BMI_confirm_calculate;
	DecimalFormat decimalFormat;

	private Spinner sp_sex;
	private String sex="男";
	private double waist_circumference = 0.0, weight = 0.0, data = 0.0;
	...
	public void initView() {
		...
		tv_BFR_date = (TextView) findViewById(R.id.tv_BFR_date);
		tv_BFR = (TextView) findViewById(R.id.tv_BFR);
		edi_BMI_weight = (EditText) findViewById(R.id.edi_BMI_weight);
		edi_BFR_waist_circumference = (EditText) findViewById(R.id.edi_BFR_waist_circumference);
	...
	public void startGame() {
		...
		UserBodyDataAPI.getUserBodyData(context, AbSharedUtil.getString(context, Constant.LAST_LOGINID), "BFR",
	...
	private OnClickListener onClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.btn_BMI_confirm_calculate:
				...
				if (edi_BFR_waist_circumference.getText().toString().isEmpty()) {
					AbToastUtil.showToast(context, "腰围不能为空！");
					return;
				}
				weight = Double.parseDouble(edi_BMI_weight.getText().toString());
				waist_circumference = Double.parseDouble(edi_BFR_waist_circumference.getText().toString());

				if(sex.equals("男")){
					data=(((waist_circumference*0.74)-((weight*0.082)+44.74))/weight)*100;
				}else{
					data=(((waist_circumference*0.74)-((weight*0.082)+34.89))/weight)*100;
				}
				
				String date = DateUtil.getDateEN();

				UserBodyDataAPI.addUserBodyData(context, AbSharedUtil.getString(context, Constant.LAST_LOGINID), "BFR",
						decimalFormat.format(data), decimalFormat.format(waist_circumference), decimalFormat.format(weight), date,
						addUserBodyDataStringHttpResponseListener);
				break;
	...
	private LineData LineData(int count) {
		...
		mlinechart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
			...
			public void onValueSelected(Entry e, int dataSetIndex, Highlight h) {
				// TODO Auto-generated method stub
				tv_BFR_date.setText(userBodyDataPoList.get(e.getXIndex()).getDate());
				tv_BFR.setText(userBodyDataPoList.get(e.getXIndex()).getData());
		...
		// y轴的数据集
		LineDataSet set = new LineDataSet(y, "BFR数据");
		// 用y轴的集合来设置参数
		// 线宽
		...
		public void onSuccess(int statusCode, String content) {
			...
			if (!StringUtil.isEmpty(content)) {
				try {
					...
					tv_BFR_date.setText(userBodyDataPoList.get(userBodyDataPoList.size() - 1).getDate());
					tv_BFR.setText(userBodyDataPoList.get(userBodyDataPoList.size() - 1).getData()+"%");
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		...
		public void onSuccess(int statusCode, String content) {
			...
			if (!StringUtil.isEmpty(content)) {
				try {
					...
					UserBodyDataAPI.getUserBodyData(context, AbSharedUtil.getString(context, Constant.LAST_LOGINID),
							"BFR", getUserBodyDataStringHttpResponseListener);
