 ...
 * @Description: TODO(与Fargment组合使用的导航栏的布局View)
 * @author 康良涛
 * @date 2016年12月15日 上午8:41:39
 *
 */
public class MainSlidingTabLayout extends HorizontalScrollView {
	...
	private static final int TITLE_OFFSET_DIPS = 24;// 选项卡的位移长度
	private static final int TAB_VIEW_PADDING_DIPS = 16; // 选项卡中textview的间距
	private static final int TAB_VIEW_TEXT_SIZE_SP = 12;// 选项卡中textview文字的大小
	...
	public List<Bitmap> bitmapPress;
	public List<Bitmap> bitmapUnPress;
	 ...
	 * When using {@link MainSlidingTabLayout} you are required to set any
	...
	private void populateTabStrip() {
		...
		for (int i = 0; i < adapter.getCount(); i++) {
			...
			ImageView tabTitleImageView = null;
			...
			if (mTabViewLayoutId != 0) {
				...
				tabTitleImageView = (ImageView) tabView.findViewById(mTabViewImageViewId);
			...
			if (tabTitleImageView == null && ImageView.class.isInstance(tabView)) {
				tabTitleImageView = (ImageView) tabView;
			}

			// 把得到的图片绑定在控件上显示
			tabTitleImageView.setImageBitmap(bitmapUnPress.get(i));
			imageViewList.add(tabTitleImageView);
		...
		MyApplication.imageViewList=imageViewList;
	}
		...
		public void onClick(View v) {
			for (int i = 0; i < mTabStrip.getChildCount(); i++) {
				if (v == mTabStrip.getChildAt(i)) {
					...
					// 把得到的图片绑定在控件上显示
					imageViewList.get(i).setImageBitmap(bitmapPress.get(i));
					for (int x = 0; x < imageViewList.size(); x++) {
						if (x != i) {
							imageViewList.get(x).setImageBitmap(bitmapUnPress.get(x));
						}
	...
	public void initBitmap() {
		bitmapPress = new ArrayList<Bitmap>();
		bitmapUnPress = new ArrayList<Bitmap>();
		InputStream in;
		
		in=context.getResources().openRawResource(R.drawable.recommended_unpress);
		myBitmap = BitmapFactory.decodeStream(in);
		bitmapUnPress.add(myBitmap);

		in=context.getResources().openRawResource(R.drawable.fitness_unpress);
		myBitmap = BitmapFactory.decodeStream(in);
		bitmapUnPress.add(myBitmap);

		in=context.getResources().openRawResource(R.drawable.dynamic_unpress);
		myBitmap = BitmapFactory.decodeStream(in);
		bitmapUnPress.add(myBitmap);

		in=context.getResources().openRawResource(R.drawable.personal_unpress);
		myBitmap = BitmapFactory.decodeStream(in);
		bitmapUnPress.add(myBitmap);

		in=context.getResources().openRawResource(R.drawable.recommended_press);
		myBitmap = BitmapFactory.decodeStream(in);
		bitmapPress.add(myBitmap);

		in=context.getResources().openRawResource(R.drawable.fitness_press);
		myBitmap = BitmapFactory.decodeStream(in);
		bitmapPress.add(myBitmap);

		in=context.getResources().openRawResource(R.drawable.dynamic_press);
		myBitmap = BitmapFactory.decodeStream(in);
		bitmapPress.add(myBitmap);

		in=context.getResources().openRawResource(R.drawable.personal_press);
		myBitmap = BitmapFactory.decodeStream(in);
		bitmapPress.add(myBitmap);
		...
		MyApplication.bitmapPress=bitmapPress;
		MyApplication.bitmapUnPress=bitmapUnPress;
