...
import lyu.klt.graduationdesign.view.ImageProcessing;
import lyu.klt.graduationdesign.view.RingView;

/*
 * 心率检测
 */
public class HeartRateDetectionActivity extends Activity {
	// 曲线
	private Timer timer;
	...
	private static Button startHeartBeatTest = null;
	...
	private static RingView mringView;
	private static TextView tv_nowHeartRate;

	public static List<Integer> heartRateList = new ArrayList<Integer>();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_heart_rate_detection_layout);
		// 曲线
		...
		startHeartBeatTest = (Button) findViewById(R.id.startHeartBeatTest);
		mringView = (RingView) findViewById(R.id.ringView);
		tv_nowHeartRate = (TextView) findViewById(R.id.tv_nowHeartRate);
		...
		startHeartBeatTest.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
					...
					startHeartBeatTest.setText("正在检测");
					startHeartBeatTest.setEnabled(false);
					mringView.startAnim();
					camera.startPreview();
		...
		title_bar_text.setText("心率检测");
		title_bar_text.setTextColor(android.graphics.Color.parseColor("#FFFFFFFF"));
		title_bar_left_img.setOnClickListener(new OnClickListener() {
	...
	private void updateChart() {
		...
		if (flag == 1)
			...
			if (gx < 200) {
				if (hua[20] > 1) {
					Toast.makeText(HeartRateDetectionActivity.this, "请用您的指尖盖住摄像头镜头！", Toast.LENGTH_SHORT).show();
					hua[20] = 0;
				}
				hua[20]++;
				return;
	...
	private PreviewCallback previewCallback = new PreviewCallback() {
		...
		public void onPreviewFrame(byte[] data, Camera cam) {
			...
			//	tv_nowHeartRate.setText("您的当前心率是"+String.valueOf(beatsAvg)+"zhi:"+String.valueOf(beatsArray.length)+" "+String.valueOf(beatsIndex)+" "+String.valueOf(beatsArrayAvg)+""+String.valueOf(beatsArrayCnt));
				tv_nowHeartRate.setText(String.valueOf(beatsAvg));
				heartRateList.add(beatsAvg);
				if(heartRateList.size()>5){
					int x=heartRateList.get(heartRateList.size()/2-1)-heartRateList.get(heartRateList.size()-1);
					if(x>-3&&x<3){
						startHeartBeatTest.setText("检测完成");
						//	startHeartBeatTest.setEnabled(false);
							mringView.stopAnim();
							if(timer!=null){
								// 当结束程序时关掉Timer
